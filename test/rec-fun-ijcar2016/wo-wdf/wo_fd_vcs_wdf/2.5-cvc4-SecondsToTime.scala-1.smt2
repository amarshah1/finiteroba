(set-info :smt-lib-version 2.6)
(set-logic AUFBVDTLIA)
(set-info :source |
Generated by: Andrew Reynolds
Generated on: 2017-04-28
Generator: Nunchaku, Leon, CVC4, converted to v2.6 by CVC4
Application: Counterexample generation for higher-order theorem provers
Target solver: CVC4, Z3
Publications: "Model Finding for Recursive Functions in SMT" by Andrew Reynolds, Jasmin Christian Blanchette, Simon Cruanes, and Cesare Tinelli, IJCAR 2016.
|)
(set-info :license "https://creativecommons.org/licenses/by/4.0/")
(set-info :category "industrial")
(set-info :status unknown)


(declare-datatypes ((tuple3!899 0)) (((tuple3!899!900 (_1!901 Int) (_2!902 Int) (_3!903 Int)))
))
(declare-fun rec!222 (Int Int Int Int) tuple3!899)
(declare-fun propSum!208 (Int Int Int Int) Bool)
(declare-fun prop!213 (Int Int Int Int) Bool)
(declare-sort I_rec!222 0)
(declare-fun rec!222_arg_0_1 (I_rec!222) Int)
(declare-fun rec!222_arg_1_2 (I_rec!222) Int)
(declare-fun rec!222_arg_2_3 (I_rec!222) Int)
(declare-fun rec!222_arg_3_4 (I_rec!222) Int)
(declare-sort I_propSum!208 0)
(declare-fun propSum!208_arg_0_5 (I_propSum!208) Int)
(declare-fun propSum!208_arg_1_6 (I_propSum!208) Int)
(declare-fun propSum!208_arg_2_7 (I_propSum!208) Int)
(declare-fun propSum!208_arg_3_8 (I_propSum!208) Int)
(declare-sort I_prop!213 0)
(declare-fun prop!213_arg_0_9 (I_prop!213) Int)
(declare-fun prop!213_arg_1_10 (I_prop!213) Int)
(declare-fun prop!213_arg_2_11 (I_prop!213) Int)
(declare-fun prop!213_arg_3_12 (I_prop!213) Int)
(assert (forall ((?i I_rec!222)) (and (= (rec!222 (rec!222_arg_0_1 ?i) (rec!222_arg_1_2 ?i) (rec!222_arg_2_3 ?i) (rec!222_arg_3_4 ?i)) (ite (>= (rec!222_arg_1_2 ?i) 3600) (rec!222 (rec!222_arg_0_1 ?i) (+ (- 3600) (rec!222_arg_1_2 ?i)) (+ 1 (rec!222_arg_2_3 ?i)) (rec!222_arg_3_4 ?i)) (ite (>= (rec!222_arg_1_2 ?i) 60) (rec!222 (rec!222_arg_0_1 ?i) (+ (- 60) (rec!222_arg_1_2 ?i)) (rec!222_arg_2_3 ?i) (+ 1 (rec!222_arg_3_4 ?i))) (tuple3!899!900 (rec!222_arg_2_3 ?i) (rec!222_arg_3_4 ?i) (rec!222_arg_1_2 ?i))))) (ite (>= (rec!222_arg_1_2 ?i) 3600) (not (forall ((?z I_rec!222)) (not (and (= (rec!222_arg_0_1 ?z) (rec!222_arg_0_1 ?i)) (= (rec!222_arg_1_2 ?z) (+ (- 3600) (rec!222_arg_1_2 ?i))) (= (rec!222_arg_2_3 ?z) (+ 1 (rec!222_arg_2_3 ?i))) (= (rec!222_arg_3_4 ?z) (rec!222_arg_3_4 ?i)))) )) (ite (>= (rec!222_arg_1_2 ?i) 60) (not (forall ((?z I_rec!222)) (not (and (= (rec!222_arg_0_1 ?z) (rec!222_arg_0_1 ?i)) (= (rec!222_arg_1_2 ?z) (+ (- 60) (rec!222_arg_1_2 ?i))) (= (rec!222_arg_2_3 ?z) (rec!222_arg_2_3 ?i)) (= (rec!222_arg_3_4 ?z) (+ 1 (rec!222_arg_3_4 ?i))))) )) true))) ))
(assert (forall ((?i I_propSum!208)) (= (propSum!208 (propSum!208_arg_0_5 ?i) (propSum!208_arg_1_6 ?i) (propSum!208_arg_2_7 ?i) (propSum!208_arg_3_8 ?i)) (= (propSum!208_arg_0_5 ?i) (+ (* 3600 (propSum!208_arg_1_6 ?i)) (* 60 (propSum!208_arg_2_7 ?i)) (propSum!208_arg_3_8 ?i)))) ))
(assert (forall ((?i I_prop!213)) (and (= (prop!213 (prop!213_arg_0_9 ?i) (prop!213_arg_1_10 ?i) (prop!213_arg_2_11 ?i) (prop!213_arg_3_12 ?i)) (and (not (>= (prop!213_arg_3_12 ?i) 60)) (>= (prop!213_arg_3_12 ?i) 0) (not (>= (prop!213_arg_2_11 ?i) 60)) (>= (prop!213_arg_2_11 ?i) 0) (propSum!208 (prop!213_arg_0_9 ?i) (prop!213_arg_1_10 ?i) (prop!213_arg_2_11 ?i) (prop!213_arg_3_12 ?i)))) (not (forall ((?z I_propSum!208)) (not (and (= (propSum!208_arg_0_5 ?z) (prop!213_arg_0_9 ?i)) (= (propSum!208_arg_1_6 ?z) (prop!213_arg_1_10 ?i)) (= (propSum!208_arg_2_7 ?z) (prop!213_arg_2_11 ?i)) (= (propSum!208_arg_3_8 ?z) (prop!213_arg_3_12 ?i)))) ))) ))
(assert (exists ((total!214 Int)) (not (=> (>= total!214 (- 0)) (or (prop!213 total!214 (_1!901 (rec!222 total!214 total!214 (- 0) (- 0))) (_2!902 (rec!222 total!214 total!214 (- 0) (- 0))) (_3!903 (rec!222 total!214 total!214 (- 0) (- 0)))) (forall ((?z I_prop!213)) (not (and (= (prop!213_arg_0_9 ?z) total!214) (= (prop!213_arg_1_10 ?z) (_1!901 (rec!222 total!214 total!214 (- 0) (- 0)))) (= (prop!213_arg_2_11 ?z) (_2!902 (rec!222 total!214 total!214 (- 0) (- 0)))) (= (prop!213_arg_3_12 ?z) (_3!903 (rec!222 total!214 total!214 (- 0) (- 0)))))) ) (forall ((?z I_rec!222)) (not (and (= (rec!222_arg_0_1 ?z) total!214) (= (rec!222_arg_1_2 ?z) total!214) (= (rec!222_arg_2_3 ?z) (- 0)) (= (rec!222_arg_3_4 ?z) (- 0)))) ) (forall ((?z I_rec!222)) (not (and (= (rec!222_arg_0_1 ?z) total!214) (= (rec!222_arg_1_2 ?z) total!214) (= (rec!222_arg_2_3 ?z) (- 0)) (= (rec!222_arg_3_4 ?z) (- 0)))) ) (forall ((?z I_rec!222)) (not (and (= (rec!222_arg_0_1 ?z) total!214) (= (rec!222_arg_1_2 ?z) total!214) (= (rec!222_arg_2_3 ?z) (- 0)) (= (rec!222_arg_3_4 ?z) (- 0)))) )))) ))
(check-sat)
(exit)

