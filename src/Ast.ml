...
let rec pp_ty out (ty : ty) =
  ...
  | Ty_bv n -> pp_str out ("(_ BitVec " ^ string_of_int n ^ ")")
  | Ty_array (t1, t2) ->
     fpf out "(@[Array@ %a@ %a@])" pp_ty t1 pp_ty t2
...
let str_of_bitvec = function
  ...
  | BV2nat -> "bv2nat"
  | BVUge -> "bvuge"
  | BVconst (i, n) -> "_ bv" ^ i ^ " " ^ (string_of_int n) 
  (* | BVconst (i, n) -> "#b" ^ int_to_bitvec n i *)
...
let pp_array out a = Format.pp_print_string out (str_of_array a)
...
let rec pp_term lvl out (t : term) =
  ...
  | Array (op, l) -> 
    Format.fprintf out "(@[<hv>%a@ %a@])" pp_array op (pp_list self_a) l
